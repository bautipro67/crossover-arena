...
func _siguiente_charla() -> void:
	...
	var nombre := Historia.nombre_de(personaje)
	var mision := Modos.mision as MisionHistoria
	if mision != null and mision.participantes.has(quien):
		nombre = mision.nombre_de(quien)
	_charla_nombre.text = nombre.to_upper() if quien != Historia.NARRADOR else ""
